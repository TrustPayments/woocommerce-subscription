...
/**
 *
 * WC_TrustPayments_Subscription_Gateway Class
 *
 * TrustPayments
...
/**
 * This class implements the Trust Payments subscription gateways
 */
class WC_TrustPayments_Subscription_Gateway {

	/**
	 * Gateway.
	 *
	 * @var WC_TrustPayments_Gateway $gateway Gateway.
	 */
	private $gateway;
	 ...
	 */
	public function __construct( WC_TrustPayments_Gateway $gateway ) {
	...
	public function process_scheduled_subscription_payment( $amount_to_charge, WC_Order $order ) {
		try {
			$token_space_id = get_post_meta( $order->get_id(), '_trustpayments_subscription_space_id', true );
			$token_id = get_post_meta( $order->get_id(), '_trustpayments_subscription_token_id', true );

			if ( empty( $token_space_id ) || get_option( WooCommerce_TrustPayments::CK_SPACE_ID ) != $token_space_id ) {
				$order->update_status( 'failed', __( 'The token space and the configured space are not equal.', 'woo-trustpayments-subscription' ) );
				return;
			}
			if ( empty( $token_id ) ) {
				$order->update_status( 'failed', __( 'There is no token associated with this subscription.', 'woo-trustpayments-subscription' ) );
				return;
			}
			$transaction_service = WC_TrustPayments_Subscription_Service_Transaction::instance();
			...
			if ( $transaction_info->get_id() > 0 ) {
				...
				if ( $existing_transaction->getState() != \TrustPayments\Sdk\Model\TransactionState::PENDING ) {
					return;
				}
			...
			$order->add_meta_data( '_trustpayments_gateway_id', $this->gateway->id, true );
			$order->delete_meta_data( '_wc_trustpayments_restocked' );
		} catch ( Exception $e ) {
			$order->update_status( 'failed', $e->getMessage(), 'woo-trustpayments-subscription' );
			WooCommerce_TrustPayments_Subscription::instance()->log( $e->getMessage() . "\n" . $e->getTraceAsString() );
			return;
		}
	}
	...
	public function update_payment_method( $update, $new_payment_method, $subscription ) {
		if ( $this->gateway->id == $new_payment_method ) {
			...
			add_filter(
				'wc_trustpayments_gateway_result_send_json',
				array(
					$this,
	...
	public function store_gateway_result_in_globals( $result, $subscription ) {
		if ( isset( $result['trustpayments'] ) ) {
			$GLOBALS['_wc_trustpayments_subscription_gateway_result'] = $result;
			return array(
				'result' => $result['result'],
				'redirect' => 'wc_trustpayments_subscription_redirect',
			);
		}
		return $result;
	}
	...
	public function create_json_response( $location, $status ) {
		if ( 'wc_trustpayments_subscription_redirect' == $location && isset( $GLOBALS['_wc_trustpayments_subscription_gateway_result'] ) ) {
			wp_send_json( $GLOBALS['_wc_trustpayments_subscription_gateway_result'] );
			exit;
		}
		return $location;
	}
		...
		$payment_meta[ $this->gateway->id ] = array(
			'post_meta' => array(
				'_trustpayments_subscription_space_id' => array(
					'value' => get_post_meta( $subscription->get_id(), '_trustpayments_subscription_space_id', true ),
					'label' => 'Trust Payments Space Id',
				),
				'_trustpayments_subscription_token_id' => array(
					'value' => get_post_meta( $subscription->get_id(), '_trustpayments_subscription_token_id', true ),
	...
	public function validate_subscription_payment_meta( $payment_method_id, $payment_meta ) {
		...
		if ( $this->gateway->id === $payment_method_id ) {
			if ( ! isset( $payment_meta['post_meta']['_trustpayments_subscription_space_id']['value'] ) || empty( $payment_meta['post_meta']['_trustpayments_subscription_space_id']['value'] ) ) {
				throw new Exception( __( 'The Trust Payments Space Id value is required.', 'woo-trustpayments-subscription' ) );
			} elseif ( get_option( WooCommerce_TrustPayments::CK_SPACE_ID ) != $payment_meta['post_meta']['_trustpayments_subscription_space_id']['value'] ) {
				throw new Exception( __( 'The Trust Payments Space Id needs to be in the same space as configured in the main configuration.', 'woo-trustpayments-subscription' ) );
			}
			if ( ! isset( $payment_meta['post_meta']['_trustpayments_subscription_token_id']['value'] ) || empty( $payment_meta['post_meta']['_trustpayments_subscription_token_id']['value'] ) ) {
				throw new Exception( __( 'The Trust Payments Token Id value is required.', 'woo-trustpayments-subscription' ) );
			}
		}
	}
	...
	public function process_subscription_failing_payment_method_updated( $subscription, $renewal_order ) {
		update_post_meta( $subscription->get_id(), '_trustpayments_subscription_space_id', $renewal_order->get_meta( '_trustpayments_subscription_space_id', true ) );
		update_post_meta( $subscription->get_id(), '_trustpayments_subscription_token_id', $renewal_order->get_meta( '_trustpayments_subscription_token_id', true ) );
	}
