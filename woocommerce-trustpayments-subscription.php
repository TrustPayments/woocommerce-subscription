...
if (! class_exists('WooCommerce_TrustPayments_Subscription')) {
    ...
    final class WooCommerce_TrustPayments_Subscription
    {
        ...
        public function loaded()
        {
            ...
            add_filter('wc_trustpayments_enhance_gateway', array(
                $this,
                'enhance_gateway'
            ));
            add_filter('wc_trustpayments_modify_sesion_create_transaction', array(
                $this,
                'update_transaction_from_session'
            ));
            add_filter('wc_trustpayments_modify_session_pending_transaction', array(
                $this,
                'update_transaction_from_session'
            ));
            add_filter('wc_trustpayments_modify_order_create_transaction', array(
                $this,
                'update_transaction_from_order'
            ), 10, 2);
            add_filter('wc_trustpayments_modify_order_pending_transaction', array(
                $this,
                'update_transaction_from_order'
            ), 10, 2);
            add_filter('wc_trustpayments_modify_confirm_transaction', array(
                $this,
                'update_transaction_from_order'
            ),10, 2);
            
            add_filter('wc_trustpayments_modify_line_item_order', array(
                $this,
                'modify_line_item_method_change'
            ), 10, 2);
            
            add_filter( 'wc_trustpayments_modify_total_to_check_order', array(
                $this, 'modify_order_total_method_change'
            ), 12, 2);
  
            add_action('wc_trustpayments_authorized', array(
                $this,
                'update_subscription_data'
            ), 10, 2);
            add_action('wc_trustpayments_fulfill', array(
                $this,
                'fulfill_in_progress'
            ), 10, 2);
            ...
            add_filter('wc_trustpayments_update_transaction_info', array(
                $this,
                'update_transaction_info'
            ), 10,3);
            add_filter('wc_trustpayments_confirmed_status', array(
                $this, 'ignore_status_update_for_subscription'
            ), 10, 2);
            add_filter('wc_trustpayments_authorized_status', array(
                $this, 'ignore_status_update_for_subscription'
            ), 10, 2);
            add_filter('wc_trustpayments_completed_status', array(
                $this, 'ignore_status_update_for_subscription'
            ), 10, 2);
            add_filter('wc_trustpayments_decline_status', array(
                $this, 'ignore_status_update_for_subscription'
            ), 10, 2);
            add_filter('wc_trustpayments_failed_status', array(
                $this, 'ignore_status_update_for_subscription'
            ), 10, 2);
            add_filter('wc_trustpayments_voided_status', array(
                $this, 'ignore_status_update_for_subscription'
            ), 10, 2);
            ...
            add_filter('wc_trustpayments_is_method_available', array(
                $this, 'method_available_for_cart_renewal'
            ), 10, 1);
            
            add_filter('wc_trustpayments_success_url', array(
                $this, 'update_success_url'
            ), 10, 2);
            add_filter('wc_trustpayments_checkout_failure_url', array(
